...
// --- Instructions (Copies from extractors) ---
// UPGRADED: Using the full, detailed prompt from src/extractors/actaCompanyIdentity.ts
...
const TELMEX_INSTRUCTIONS = `You are a strict KYC extractor for Mexican Telmex bills. Extract ProofOfAddress.
GLOBAL HARDENING RULES:
- Never infer or generate data.
- Normalize all dates to YYYY-MM-DD.
- Convert amounts to numeric values.`;
...
const docs = [
  ...
  { type: "telmex" as DocumentType,         fileUrl: resolveFixture("Recibo-Oct (2).pdf"), instructions: TELMEX_INSTRUCTIONS, schema: ProofOfAddressSchema },
